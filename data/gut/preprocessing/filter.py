...
for p_id in np.unique(p_ids):
    ...
    if patient_days.size < 2:
        passed = [False] * patient_days.size
        keep = keep + passed
        continue

    # clip observations too far in the future or start day is too far
    day_dist = 8
    passed = [True] * patient_days.size
    for i in range(1, patient_days.size - 1):
        k = True
        if patient_days[i] - patient_days[i - 1] > day_dist and patient_days[i + 1] - patient_days[i] > day_dist:
            k = False
        passed[i] = k
    passed[0]  = patient_days[1] - patient_days[0] < day_dist
    passed[-1] = patient_days[-1] - patient_days[-2] < day_dist

    # remove if too sparse
    if np.sum(np.array(passed).astype(int)) < 5:  #  or average_density(patient_days[passed]) > 1
        passed = [False] * patient_days.size
        keep = keep + passed
        continue
...
print(np.unique(taur[0]).size - 1, "patients remaining")
